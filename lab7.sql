...
-- E
SELECT SDO_LRS.PROJECT_PT(S6.GEOM, SDO_GEOMETRY(2001, 8307, SDO_POINT_TYPE(T.X, T.Y, NULL), NULL, NULL)) AS WJAZD_NA_S6
FROM S6_LRS S6,
     MAJOR_CITIES C,
     TABLE(SDO_UTIL.GETVERTICES(S6.GEOM)) T
WHERE C.CITY_NAME = 'Slupsk'
  AND T.X < C.GEOM.SDO_POINT.X
ORDER BY SDO_GEOM.SDO_DISTANCE(C.GEOM, SDO_GEOMETRY(2001, 8307, SDO_POINT_TYPE(T.X, T.Y, NULL), NULL, NULL), 1, 'unit=km') ASC
